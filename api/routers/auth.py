...
def auth_login(payload: LoginIn):
    ...
    # erro genérico para evitar enumeração de usuário
    if not row:
        raise HTTPException(status_code=401, detail="Credenciais inválidas")

    if not bool(row._mapping.get("is_active", True)):
        raise HTTPException(status_code=401, detail="Credenciais inválidas")
...
def get_me(user_id: int = Depends(get_current_user)):
    """
    Retorna os dados do usuário autenticado (fonte da verdade da sessão).

    Use este endpoint para o frontend validar a sessão no carregamento.
    """
    eng = get_engine()
    with eng.connect() as conn:
        row = conn.execute(
            text("SELECT id, email, name, role, is_active FROM eta.app_user WHERE id=:id"),
            {"id": user_id},
        ).fetchone()

    # Em tese get_current_user já valida, mas mantemos por segurança/robustez
    if not row:
...
def validate_invite(token: str):
    ...
    with eng.connect() as conn:
        row = conn.execute(
            text("SELECT email, expires_at, used FROM eta.user_invites WHERE token = :t"),
            {"t": token},
        ).fetchone()
    ...
    if row._mapping["used"] or datetime.utcnow() > row._mapping["expires_at"]:
...
def register_with_invite(payload: RegisterInviteIn):
    ...
    with eng.connect() as conn:
        inv = conn.execute(
            text("SELECT email, used, expires_at FROM eta.user_invites WHERE token=:t"),
            {"t": payload.token},
        ).fetchone()
    ...
    with eng.begin() as conn:
        ...
        conn.execute(
            text("UPDATE eta.user_invites SET used=TRUE WHERE token=:t"),
            {"t": payload.token},
        )
...
def list_users(uid: int = Depends(get_current_admin)):
    ...
    with eng.connect() as conn:
        rows = conn.execute(
            text("SELECT id, name, email, role, is_active FROM eta.app_user ORDER BY id")
        ).fetchall()
